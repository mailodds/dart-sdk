...
class OAuth20Api {
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> oauthRegisterClientWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/oauth/register';
  ...
  ///
  /// Dynamic Client Registration (RFC 7591). Allows MCP clients to auto-register without user interaction.
  Future<OAuthClientRegistration?> oauthRegisterClient() async {
    final response = await oauthRegisterClientWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'OAuthClientRegistration',) as OAuthClientRegistration;
